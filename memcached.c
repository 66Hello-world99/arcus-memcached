...
static void conn_coll_eitem_free(conn *c)
{
    switch (c->coll_op) {
      ...
      case OPERATION_JOP_GET:
        mc_engine.v1->json_elem_release(mc_engine.v0, &c->coll_eitem);
        if (c->coll_resps != NULL) {
            free(c->coll_resps); c->coll_resps = NULL;
        }
        break;
...
static void process_jop_set_complete(conn *c)
{
    ...
    if (einfo_check_tail_crlf(&c->einfo) != 0) { /* check "\r\n" */
        ...
        default:
            ...
            else if (ret == ENGINE_EBADVALUE) out_string(c, "PATH PARSING ERROR");
                             ...
                             item_attr *attrp)
{
    ...
    if (ret == ENGINE_SUCCESS) {
        ...
        ret = mc_engine.v1->json_elem_dfs(elem_item, &respbuf, &len);
    }
